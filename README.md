...
```
index.html            корни виджетов и порядок подключения скриптов
...
Первый убеждается, что все пути из `index.html`, JS и списка прекеша `sw.js`
существуют и что каждый скрипт парсится. Второй грузит страницу в jsdom с
заглушками холста и звука и прогоняет меню, хаб, забег, босса, левелап и смерть —
ловит битый порядок подключения, обращения к несуществующим именам и виджеты,
которые забыли вставить свою разметку.

jsdom намеренно не лежит в проекте, ставится разово во временную папку:

```
npm install jsdom --prefix %TEMP%\consultant-smoke
```

Список прекеша в `sw.js` генерируется, руками его править не нужно:

```
node tools/sync-sw-precache.mjs
```
